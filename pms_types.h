...
struct PVector3f {
  ...
  // operator -t
  PVector3f operator-() const { return PVector3f(-x, -y, -z); }
  // operator =
  ...
  // dot
  float32 dot(const PVector3f& v) const { return x * v.x + y * v.y + z * v.z; }
};
...
struct DisparityPlane {
   ...
   * @param d 视差
   */
  DisparityPlane(const sint32& x, const sint32& y, const PVector3f& n,
                 const float32& d) {
    p.x = -n.x / n.z;
    p.y = -n.y / n.z;
   ...
   * 代入左视图视差平面方程就可得到右视图坐标系下的平面方程: d = a_p/(a_p-1)*xr
   * + b_p/(a_p-1)*yr + c_p/(a_p-1) 右至左同理
   * \param x		像素x坐标
   * \param y 	像素y坐标
   * \return 转换后的平面
   */
